...
class AFlowNet(nn.Module):
    ...
    def forward(self, x, x_warps, x_conds, warp_feature=True):
        ...
        for i in range(len(x_warps)):
          ...
          corr_my = CorrTorch(max_displacement=3,pad_size=3)
          tenCorrelation = F.leaky_relu(input=corr_my(x_warp_after,x_cond), negative_slope=0.1, inplace=False)
...
class AFWM(nn.Module):
    ...
    def __init__(self, input_nc, opt=BaseOptions().parse()):
